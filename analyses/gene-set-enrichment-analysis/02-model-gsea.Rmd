...
scores_file <- readr::read_tsv(scores_file) %>% 
  dplyr::filter(data_type %in% c("polya", "stranded"))
```
...
#   dplyr::select(Kids_First_Biospecimen_ID, cancer_group, cancer_order, hex_codes) %>% 
#   # Reorder cancer_group based on cancer_order
#   dplyr::mutate(cancer_group = forcats::fct_reorder(cancer_group, cancer_order))
