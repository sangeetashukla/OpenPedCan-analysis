...
rna_library_list <- metadata %>% pull(RNA_library) %>% unique()
...
harmonized_diagnosis_anova_outpaths <- lapply(rna_library_list, function(x){
  file.path(results_dir, paste0("gsva_anova_", x, "_harmonized_diagnosis.tsv"))
})

harmonized_diagnosis_tukey_outpaths <- lapply(rna_library_list, function(x){
  file.path(results_dir, paste0("gsva_tukey_", x, "_harmonized_diagnosis.tsv"))
})
...
for(i in 1:length(rna_library_list)){
  ...
  harmonized_diagnosis_model_results <- gsva_anova_tukey(metadata_with_gsva, harmonized_diagnosis, rna_library, SIGNIFICANCE_THRESHOLD)
  
  ## Save stranded library results
  write_tsv(cancer_group_model_results[["anova"]], cancer_group_anova_outpaths[i])
  write_tsv(harmonized_diagnosis_model_results[["anova"]], harmonized_diagnosis_anova_outpaths[i])
  write_tsv(cancer_group_model_results[["tukey"]],  cancer_group_tukey_outpaths[i])
  write_tsv(harmonized_diagnosis_model_results[["tukey"]], harmonized_diagnosis_anova_outpaths[i])
  
  print(rna_library)
  print(head(cancer_group_model_results))
  print(head(harmonized_diagnosis_model_results))
}
...
for(i in 1:length(rna_library_list)){
  ...
  print(paste0("Number of cancer groups having significant ANOVAs across hallmark pathways for RNA library ", rna_library, " ", num_sig))
}
...
for(i in 1:length(rna_library_list)){
  ...
  num_sig <- harmonized_diagnosis_model_results[["anova"]] %>% 
    count(significant_anova) 
  
  # print out information
  print(paste0("Number of harmonized diagnosis having significant ANOVAs across hallmark pathways for RNA library ", rna_library, " ", num_sig))
