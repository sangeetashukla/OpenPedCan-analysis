...
TARGET <- TARGET %>% 
  select(Kids_First_Biospecimen_ID, Kids_First_Participant_ID, sample_id, cohort, experimental_strategy)
GMKF <- GMKF %>% 
  select(Kids_First_Biospecimen_ID, Kids_First_Participant_ID, sample_id, cohort, experimental_strategy)
```
...
```{r assign QC values}
QC_df <- QC_df %>% mutate(ID = row_number())
values_from_subtype_table <- values_from_subtype_table %>% mutate(ID = row_number())
QC_df <- inner_join(QC_df,values_from_subtype_table,by="ID")
QC_df$ID<-NULL
```
