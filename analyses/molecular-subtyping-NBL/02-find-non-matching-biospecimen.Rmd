...
date: "10/13/2022"
---
In this notebook we find the find the biospecimens which do not have matched DNA and RNA IDs. 
We create a dataframe for these biospecimens called `MYCN_non_match_df` which has all the necessary
infromation required for performing subtyping. We concatenate this `MYCN_non_match_df` with the table
`results/mycn_nbl_matched_biospecimen.tsv` and joined table called 
`results/alteration_table_without_subtype.tsv` which contains the list of all the NBL-MYCN 
biospecimens that need to subtpyped. 

The notebook also plots the TPM vs Kids_First_Biospecimen_IDs for all the biospecimens for which there
is a TPM value. We overlay the consensus status call for each of the biospecimens in this plot and
detemine a cut off for amplification. 
...
1. mycn-nbl file (mycn_nbl_subset_data.tsv)
2. The clinical patient-status mapping files for GMKF  (gmkf_patient_clinical_mycn_status.tsv)
3. The clinical patient-status mapping files for TARGET (target_patient_clinical_mycn_status.tsv)
4. The DNA only biospecimens file
5. The RNA only biospecimens file
...
gmkf_patient_clinical_mycn_status_df <- readr::read_tsv(file.path(module_dir, "input/gmkf_patient_clinical_mycn_status.tsv"),guess_max = 100000)

target_patient_clinical_mycn_status_df<- readr::read_tsv(file.path(module_dir, "input/target_patient_clinical_mycn_status.tsv"),guess_max = 100000)

MYCN_dna_df<-readr::read_tsv(file.path(results_dir, "mycn_nbl_dna_biospecimen.tsv"),guess_max = 100000)
MYCN_rna_df<-readr::read_tsv(file.path(results_dir, "mycn_nbl_rna_biospecimen.tsv"),guess_max = 100000)
...
```{r chunk 1}
index_dna <- which(MYCN_IDs_without_DNA_RNA_Match$Kids_First_Biospecimen_ID 
                       %in% MYCN_dna_df$Kids_First_Biospecimen_ID_DNA)
```

```{r chunk 2}
DNA_IDs_MYCN_Non_Match <- MYCN_IDs_without_DNA_RNA_Match[index_dna,]
```

```{r chunk 3}
DNA_IDs_MYCN_Non_Match<- as.data.frame(DNA_IDs_MYCN_Non_Match)
```
...
index_rna<- which(MYCN_IDs_without_DNA_RNA_Match$Kids_First_Biospecimen_ID 
                       %in% MYCN_rna_df$Kids_First_Biospecimen_ID_RNA)
RNA_IDs_MYCN_Non_Match<-MYCN_IDs_without_DNA_RNA_Match[index_rna,]
RNA_IDs_MYCN_Non_Match<-as.data.frame(RNA_IDs_MYCN_Non_Match)
colnames(RNA_IDs_MYCN_Non_Match)<- "Kids_First_Biospecimen_ID_RNA"
...
DNA_only_MYCN_df <- DNA_only_MYCN_df %>% select(Kids_First_Biospecimen_ID,
                                                Kids_First_Participant_ID,MYCN_TPM,MYCN_CN,
                                                MYCN_CN_status,pathology_free_text_diagnosis)  # 763 samples 
...
index_dna_only <- which(DNA_only_MYCN_df$Kids_First_Biospecimen_ID_DNA=="TARGET-30-PAMDAL-01A-01W" 
                        & DNA_only_MYCN_df$MYCN_CN==3 &
                        DNA_only_MYCN_df$MYCN_CN_status=="neutral")
...
RNA_only_MYCN_df <- RNA_only_MYCN_df %>% select(Kids_First_Biospecimen_ID,Kids_First_Participant_ID,
                                                MYCN_TPM,MYCN_CN,MYCN_CN_status,
...
DNA_only_MYCN_df <- DNA_only_MYCN_df %>% mutate(Kids_First_Biospecimen_ID_RNA=NA_character_)
RNA_only_MYCN_df<- RNA_only_MYCN_df %>% mutate(Kids_First_Biospecimen_ID_DNA=NA_character_)
#DNA_only_MYCN_df$Kids_First_Biospecimen_ID_RNA<- NA
#RNA_only_MYCN_df$Kids_First_Biospecimen_ID_DNA<- NA
```
...
alteration_df <- MYCN_DNA_RNA_FILTERED_DF %>% select(Kids_First_Biospecimen_ID_DNA,
                                                     ...
                                                     MYCN_TPM,MYCN_CN,MYCN_TPM,MYCN_CN_status,
...
alteration_df<-alteration_df %>% select(Kids_First_Biospecimen_ID_DNA,Kids_First_Biospecimen_ID_RNA,                            
                                        MYCN_TPM,MYCN_CN,MYCN_TPM,MYCN_CN_status,pathology)
...
bar_plot2<-plot_alteration_df %>% 
  ...
  ggplot() +aes(x=Kids_First_Biospecimen_ID, y=MYCN_TPM, fill=MYCN_CN_status)+
...
```{r}
alteration_df%>%readr::write_tsv(file.path(results_dir, "alteration_table_without_molecular_subtype.tsv"))
```