...
#Part 6: QC Check
```{r perform QC check}
GMKF_MYCN_NBL_df <- MYCN_DF %>% filter(cohort=="GMKF" & experimental_strategy %in% c("WGS","WXS"))
TARGET_MYCN_NBL_df <- MYCN_DF %>% filter(cohort=="TARGET"& experimental_strategy %in% c("WGS","WXS"))
QC_df <- inner_join(GMKF_MYCN_NBL_df,TARGET_MYCN_NBL_df, by=c("Kids_First_Participant_ID"="Kids_First_Participant_ID", 
                                                              "sample_id"="sample_id"))
QC_df <-  QC_df %>% select(Kids_First_Biospecimen_ID.x,Kids_First_Biospecimen_ID.y,Kids_First_Participant_ID,sample_id,cohort.x,cohort.y,experimental_strategy.x,experimental_strategy.y,status.x,status.y)
colnames(QC_df)<- c("Kids_First_Biospecimen_ID_GMKF","Kids_First_Biospecimen_ID_TARGET","Kids_First_Participant_ID","sample_id",
                    "cohort_GMKF","cohort_TARGET","experimental_strategy_GMKF","experimental_strategy_Target",
                    "status_GMKF","status_TARGET")
...
for (iter in 1:dim(QC_df)[1]){
  ...
  QC_df$subtype_GMKF[iter]<- alteration_df$subtype[index_gmkf]
    ...
    index_target <- which(alteration_df$Kids_First_Biospecimen_ID_RNA == QC_df$Kids_First_Biospecimen_ID_TARGET[iter])
  }
