...
`input/alteration_table_without_molecular_subtype.tsv`and molecular_subtypes the biospecimens based
on the six subtyping criteria (refer to the module readme). This script generates a table 
called `results/neuroblastoma_molecular_subtypes.tsv` , which contains the molecular_subtypes for 
each of the biospecimens in the table `alteration_df`.
...
```{r}
alteration_df<-readr::read_tsv(file.path(subset_dir, "alteration_table_without_molecular_subtype.tsv"),
                               guess_max = 100000)
...
for (iter in 1: length(clincal_amplified_IDs)){
  ...
  if(length(clincal_amplified_IDs)>0){ # For CI issues 
...
for (iter in index_clinical_amplified){
  ...
  # `results/neuroblastoma_molecular_subtypes.tsv` this subtype is overwritten to NBL, to be classified. 
...
index_tpm_molecular_subtype_above<-which(is.na(alteration_df$molecular_subtype) & 
...
index_tpm_molecular_subtype_below<-which(is.na(alteration_df$molecular_subtype) & 
                                   !is.na(alteration_df$MYCN_TPM)& 
                                   alteration_df$MYCN_TPM < Suggested_Cutoff)
# if TPM is greater assign NBL, MYCN amplified
alteration_df$molecular_subtype[index_tpm_molecular_subtype_above]<-"NBL, MYCN amplified"
# if TPM is greater assign NBL, MYCN non-amplified
alteration_df$molecular_subtype[index_tpm_molecular_subtype_below]<-"NBL, MYCN non-amplified"

```

Code Logic
Case 6: If the samples are not in cases 1-5, assign the subtype as "NBL, to be classified"
```{r : Assign sample subtype to NBL, to be classified}
# Find the remaining samples that are not yet subtyped
index_case_6 <- which(is.na(alteration_df$molecular_subtype))
# Assign these samples the subtype "NBL, to be classified"
alteration_df$molecular_subtype[index_case_6]<-"NBL, to be classified"
...
TPM_index <- c(index_case_4,index_tpm_molecular_subtype_above,index_tpm_molecular_subtype_below) 
...
# find the biospecimens that need to be assigned NBL, to be classified 
index_unclassified <- which(molecular_subtype_table$molecular_subtype %in% 
                  c("Pathology-amp,MYCN_CN_status-non-amp,TPM-NA"))
molecular_subtype_table$molecular_subtype[index_unclassified] <- "NBL, to be classified"
...
alteration_df %>% 
  readr::write_tsv(file.path(results_dir, "alteration_table_with_molecular_subtype.tsv"))
df_tpm %>% 
  readr::write_tsv(file.path(results_dir, "molecular_subtypes_based_on_cutoff.tsv"))
molecular_subtype_table %>% 
  readr::write_tsv(file.path(results_dir, "neuroblastoma_molecular_subtypes.tsv"))
```

#Remove intermediate files that are not required
```{r names of intermediate files to be deleted}
# 
# file_names_intermediate<- c(paste(subset_dir, "/mycn_nbl_subset_data.tsv",sep=""),
#                             paste(subset_dir,"/mycn_nbl_matched_biospecimen.tsv",sep=""),
#                             paste(subset_dir,"/mycn_nbl_dna_biospecimen.tsv",sep=""),
#                             paste(subset_dir,"/mycn_nbl_rna_biospecimen.tsv",sep=""),
#                             paste(subset_dir,"/alteration_table_without_molecular_subtype.tsv",sep=""))

```

```{r remove the intermediate files}

# for(file_iter in 1: length(file_names_intermediate)){
#  if (file.exists(file_names_intermediate[file_iter])) {
#    print(file_names_intermediate[file_iter])
#     unlink(file_names_intermediate[file_iter])
# 
# }
# }
```
