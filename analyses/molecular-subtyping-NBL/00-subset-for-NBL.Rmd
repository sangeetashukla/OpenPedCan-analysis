...
and TPM infortmation in the table `results/mycn_nbl_subset_data.tsv`.
...
```{r rename columns to make tables informative}
MYCN_DF <- MYCN_DF %>% rename(MYCN_CN_status= status)
MYCN_DF <- MYCN_DF %>% rename(MYCN_CN= copy_number)
```


# Write the mycn-nbl biospecimen file into results dir
```{r write alteration table}
MYCN_DF%>%readr::write_tsv(file.path(input_dir, "mycn_nbl_subset_data.tsv"))
```
