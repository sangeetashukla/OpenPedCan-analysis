...
```{r CHUNK A}
#plt_file <- file.path(plot_dir, paste(clincal_amplified_IDs[1],"_chr2p",".png",sep=""))
```

```{r CHUNK B}
#plt1<-plot_chr2(cnv_cnvkit_df,clincal_amplified_IDs[1])
```
...
```{r chunk D}
#ggsave(filename = plt_file,plot =p1)
```
...
for (iter in 1: length(clincal_amplified_IDs)){
  plt_file[iter] <- file.path(plot_dir, paste(clincal_amplified_IDs[iter],"_chr2p",".png",sep=""))
  ...
  if(!is.na(clincal_amplified_IDs[iter])){
    plt1[iter]<-plot_chr2(cnv_cnvkit_df,clincal_amplified_IDs[iter])
    #ggsave(filename = plt_file,plot =plt1@ggplot)
  }
  else{
    #If clincal_amplified_IDs is NA we need to find corresponding RNA ID
    plt1[iter]<-plot_chr2(cnv_cnvkit_df,alteration_df$Kids_First_Biospecimen_ID_RNA[index_clinical_amplified[iter]])
    #ggsave(filename = plt_file,plot = plt1@ggplot)
  }
