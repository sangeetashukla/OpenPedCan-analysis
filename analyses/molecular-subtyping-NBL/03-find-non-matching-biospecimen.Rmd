...
date: "10/13/2022"
---
In this notebook we find the biospecimens which do not have matched DNA and RNA IDs. 
We create a dataframe for these biospecimens called `MYCN_non_match_df` which has all the necessary
information required for performing subtyping. We concatenate this `MYCN_non_match_df` with the table `nbl-subset/mycn_nbl_matched_biospecimen.tsv` and joined table called `nbl-subset/alteration_table_without_subtype.tsv` which contains the list of all NBL-MYCN biospecimens that need to subtpyped. 

The notebook also plots the TPM vs Kids_First_Biospecimen_IDs for all the biospecimens for which there is a TPM value. We overlay the consensus status call for each of the biospecimens in this plot and determine a cut off for amplification. 
