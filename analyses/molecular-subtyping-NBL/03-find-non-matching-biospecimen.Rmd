...
`nbl-subset/mycn_nbl_matched_biospecimen.tsv` and joined table called 
`nbl-subset/alteration_table_without_subtype.tsv` which contains the list of all the NBL-MYCN 
...
MYCN_DNA_RNA_FILTERED_DF<-readr::read_tsv(file.path(subset_dir,"mycn_nbl_matched_biospecimen.tsv"),guess_max = 100000)

gmkf_patient_clinical_mycn_status_df <- readr::read_tsv(file.path(subset_dir, "gmkf_patient_clinical_mycn_status.tsv"),guess_max = 100000)

target_patient_clinical_mycn_status_df<- readr::read_tsv(file.path(subset_dir, "target_patient_clinical_mycn_status.tsv"),guess_max = 100000)

MYCN_dna_df<-readr::read_tsv(file.path(subset_dir, "mycn_nbl_dna_biospecimen.tsv"),guess_max = 100000)
MYCN_rna_df<-readr::read_tsv(file.path(subset_dir, "mycn_nbl_rna_biospecimen.tsv"),guess_max = 100000)
...
To resolve this issue we use the files in the nbl-subset folder which are NBL MCYN clinical 
patient-status mapping files for GMKF and TARGET samples. These NA values will be integrated in the 
V12 version. 

```{r integrate new pathology free text diagnosis values from nbl-subset folder}
# Bring in the extra pathology free diagnonsis text from files in nbl-subset folder:
...
Write NBL-MYCN biospecimen to subtyped into the nbl-subset folder
```{r}
alteration_df%>%readr::write_tsv(file.path(subset_dir, "alteration_table_without_molecular_subtype.tsv"))
```