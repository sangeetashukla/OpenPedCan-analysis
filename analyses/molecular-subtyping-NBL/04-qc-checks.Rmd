...
QC_df$MYCN_TPM_GMKF<-NA
QC_df$MYCN_TPM_Target<-NA

QC_df$MYCN_status_GMKF<-NA
QC_df$MYCN_status_Target<-NA

QC_df$MYCN_subtype_GMKF<-NA
QC_df$MYCN_subtype_Target<-NA
...
for (iter in 1:dim(QC_df)[1]){
  ...
  QC_df$MYCN_TPM_GMKF[iter]<- alteration_subtyped_df$MYCN_TPM[index_gmkf]
  QC_df$MYCN_status_GMKF[iter]<- alteration_subtyped_df$MYCN_CN_status[index_gmkf]
  QC_df$molecular_subtype_GMKF[iter]<- alteration_subtyped_df$molecular_subtype[index_gmkf]
  
  if(length(index_target) ==0 ){
    index_target <- which(alteration_subtyped_df$Kids_First_Biospecimen_ID_RNA == 
                            QC_df$Kids_First_Biospecimen_ID_TARGET[iter])
  }
  QC_df$MYCN_TPM_Target[iter]<- alteration_subtyped_df$MYCN_TPM[index_target]
  QC_df$MYCN_status_Target[iter]<- alteration_subtyped_df$MYCN_CN_status[index_target]
  QC_df$molecular_subtype_Target[iter]<- alteration_subtyped_df$molecular_subtype[index_target]
}
