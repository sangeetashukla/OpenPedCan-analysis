...
folder at the end of this script as `Alteration_Table.tsv`.

The subtyping of all the samples in `Alteration_Table.tsv` is done in script 01-subtyping.Rmd.
...
Code Logic:
We will integrate the TMP and cnv data with the hist_filtered using left join. Histology file has 
all the biospecimens but we are only concerned with NBL-biospecimens, so we use the filtered version 
of histology file (`hist_filtered_df`). 
...
the data that only have DNA IDs and RNA IDs. To do this we first create and exhaustive list of all
...
DNA_IDs_MYCN_Non_Match <- as.data.frame(MYCN_IDs_without_DNA_RNA_Match
                                        [which(
                                          MYCN_IDs_without_DNA_RNA_Match$Kids_First_Biospecimen_ID 
                                               %in%
                                                 MYCN_dna_df$Kids_First_Biospecimen_ID_DNA),]) 
...
RNA_IDs_MYCN_Non_Match <- as.data.frame(MYCN_IDs_without_DNA_RNA_Match
                                        [which(
                                          MYCN_IDs_without_DNA_RNA_Match$Kids_First_Biospecimen_ID 
                                          %in%
                                            MYCN_rna_df$Kids_First_Biospecimen_ID_RNA),])  
...
alteration_df%>%readr::write_tsv(file.path(intermediate_dir, "Alteration_Table.tsv"))
```
