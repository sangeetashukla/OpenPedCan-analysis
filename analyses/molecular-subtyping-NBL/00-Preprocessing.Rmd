...
---
# Objective:
This script creates a table called `alteration_df` which contains the biospecimens for neuroblastama, ganglioneuroblastoma, and ganglioneuroma 
which need to be subptyped. This table has the following columns `Kids_First_Biospecimen_DNA_ID`, `Kids_First_Biospecimen_RNA_ID`, 
`MYCN_TPM`, `copy_number`, `status`, `pathology_free_diagnosis_text`, and `subtype`. 

This script preprocess the input files and collect all the relevant samples that need to subtpyed and add them to the table `alteration_df`. 
We write the table `alteration_df` into the intermediate folder at the end of this script. 

The subtyping of all the samples in `alteration_df` is done in script 01-subtyping.Rmd.
