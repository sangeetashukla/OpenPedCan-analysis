...
histology_df <- histology_df %>%mutate(match_id = paste(Kids_First_Participant_ID, sample_id, sep = "_"))
  
  
independent_dna <- histology_df %>%filter(Kids_First_Participant_ID %in% independent_dna_samples_df$Kids_First_Participant_ID)
  
matched_rna <- histology_df %>%filter(experimental_strategy == "RNA-Seq" | experimental_strategy == "Targeted Sequencing",
                                                  RNA_library %in% c("exome_capture", "stranded", "poly-A", "poly-A stranded"),
                                                  match_id %in% independent_dna$match_id)


matched_rna_primary <- histology_df %>%filter(experimental_strategy == "RNA-Seq" | experimental_strategy == "Targeted Sequencing",
                                              RNA_library %in% c("exome_capture", "stranded", "poly-A", "poly-A stranded"),
                                              tumor_descriptor %in% primary_descs,match_id %in% independent_dna$match_id)
#################################################################################
