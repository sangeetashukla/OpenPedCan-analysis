...
We will next prioritize RELA and YAP1 fusions based on evidence for these subgroups from Pajtler. et al. Fig.6
...
EPN_final <- EPN_final %>%
  mutate(subgroup = ifelse(subgroup == "EPN, To be classified" & tmp == TRUE, "EPN, ST RELA", subgroup)) %>%
  dplyr::select(-c(tmp))
...
EPN_final <- EPN_final %>%
  mutate(subgroup = case_when(
    ...
    subgroup == "EPN, To be classified" & (`6q_loss` > 0 | `6p_loss` > 0) & (GPBP1_expr_zscore > 3 | IFT46_expr_zscore > 3) ~ "EPN, PF B",
    TRUE ~ subgroup)
    )
