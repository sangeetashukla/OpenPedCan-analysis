...
long_tpm_results_dir <- file.path(analyses_dir, "rna-seq-expression-summary-stats/results")
...
filter_freq_table <- function(freq_file, ensg_ids, ot_ensg_ids) {
  ...
  # Long TPM tables have different column names which need to be renamed to be compatible with this function
  freq_table <- freq_table %>% dplyr::rename_with(~ case_when(. == "Gene_Ensembl_ID" ~ "targetFromSourceId", TRUE ~.))
...
#### Long TPM zscores
#### These will not generate sample_id level list of dropped ENSGs since the data is gene wise or cancer group wise
```{r}
# Remove Ensembl gene identifiers

long_tpm_mean_sd_quantile_group_wise <- filter_freq_table(
  file.path(long_tpm_results_dir, 
            "long_n_tpm_mean_sd_quantile_group_wise_zscore.tsv.gz"), ensg_ids, ot_ensg_ids) 


long_tpm_mean_sd_quantile_gene_wise <- filter_freq_table(
  file.path(long_tpm_results_dir, 
            "long_n_tpm_mean_sd_quantile_gene_wise_zscore.tsv.gz"), ensg_ids, ot_ensg_ids) 

filtered_ensg_group <- long_tpm_mean_sd_quantile_group_wise %>% 
  dplyr::filter(!targetFromSourceId %in% setdiff(ensg_ids, ot_ensg_ids)) %>% 
  dplyr::select(Gene_symbol, targetFromSourceId) %>%
  dplyr::rename(Gene_Ensembl_ID = targetFromSourceId) %>% 
  dplyr::distinct()

filtered_ensg_gene <- long_tpm_mean_sd_quantile_gene_wise %>% 
  dplyr::filter(!targetFromSourceId %in% setdiff(ensg_ids, ot_ensg_ids)) %>% 
  dplyr::select(Gene_symbol, targetFromSourceId) %>%
  dplyr::rename(Gene_Ensembl_ID = targetFromSourceId) %>% 
  dplyr::distinct()
...
# merge sample IDs to the ENSG IDs not in gencode v39
filtered_ensg_group <- filtered_ensg_group %>% 
  dplyr::left_join(long_tpm_mean_sd_quantile_group_wise, by="Gene_symbol", copy = FALSE) %>%
  dplyr::select(-targetFromSourceId)  #duplicated by Gene_Ensembl_ID

filtered_ensg_gene <- filtered_ensg_gene %>% 
  dplyr::left_join(long_tpm_mean_sd_quantile_gene_wise, by="Gene_symbol", copy = FALSE) %>%
  dplyr::select(-targetFromSourceId)  #duplicated by Gene_Ensembl_ID


# # merge OpenPedCan cohort to the ENSG IDs not in gencode v39
# filtered_ensg_group <- filtered_ensg_group %>% 
#   dplyr::left_join(sample_ids, by= "Kids_First_Biospecimen_ID")
...
# Display removed ENSEMBL IDs 
filtered_ensg_group %>% dplyr::select(Gene_Ensembl_ID, cohort) %>% 
  dplyr::distinct() %>%
  tidyr::drop_na() %>%  
  table()

filtered_ensg_gene %>% dplyr::select(Gene_Ensembl_ID, cohort) %>% 
  dplyr::distinct() %>%
  tidyr::drop_na() %>%  
  table()


# Display Diseases in removed ENSEMBL IDs that are in PMTL list
filtered_ensg_group %>% dplyr::filter(Gene_Ensembl_ID %in% pmtl_ids) %>% 
  select(Gene_Ensembl_ID, cohort, Disease) %>% 
  dplyr::distinct()


filtered_ensg_gene %>% dplyr::filter(Gene_Ensembl_ID %in% pmtl_ids) %>% 
  select(Gene_Ensembl_ID, cohort, Disease) %>% 
  dplyr::distinct()


# Remove data frames
rm(long_tpm_mean_sd_quantile_group_wise, filtered_ensg_group)
rm(long_tpm_mean_sd_quantile_gene_wise, filtered_ensg_gene)
```
