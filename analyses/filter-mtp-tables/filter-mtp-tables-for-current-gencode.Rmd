...
filter_freq_table <- function(freq_file, ensg_ids, ot_ensg_ids) {
  ...
  lookup <-c("Gene_Ensembl_ID" = "targetFromSourceId")
  freq_table %>% rename(!!! lookup[names(.) %in% lookup])
  freq_table %>% rename_with(~ case_when(. == "Gene_Ensembl_ID" ~ "targetFromSourceId",TRUE ~ .))
  freq_table %>% rename(any_of(lookup))
  freq_table %>% rename_with(~case_when(. == "Gene_Ensembl_ID" ~ "targetFromSourceId", TRUE ~ .))
  
  if ("Gene_Ensembl_ID" %in% colnames(freq_table)) {
    freq_table %>% rename("targetFromSourceId" = Gene_Ensembl_ID)
  }
