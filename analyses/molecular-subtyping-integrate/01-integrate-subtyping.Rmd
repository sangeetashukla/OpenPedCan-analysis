...
histology <- base_histology %>% 
  ...
  mutate(harmonized_diagnosis = case_when(harmonized_diagnosis %in% c("Ewings sarcoma", "Ewings Sarcoma") ~ "Ewing sarcoma",
...
histology_cg <- histology %>%
  ...
  dplyr::mutate(cancer_group = case_when(broad_histology %in% c("Benign tumor", "Other", "Non-tumor", "Pre-cancerous lesion") ~ NA_character_,
                                         pathology_diagnosis == "Other" ~ NA_character_,
  ...
  dplyr::mutate(cancer_group = case_when(
    ...
    cancer_group == "Kidney Rhabdoid Tumor" ~ "Rhabdoid tumor of the kidney",
    cancer_group %in% c("Kidney Wilms Tumor", "Wilms Tumor") ~ "Wilms tumor",
    cancer_group == "Liver Hepatocellular Carcinoma" ~ "Hepatocellular Carcinoma",
    cancer_group == "Skin Cutaneous Melanoma" ~ "Melanoma",
    cancer_group == "Testicular Germ Cell Tumor" ~ "Testicular Germ Cell tumor",
    ...
    cancer_group == "Juvenile Xanthogranuloma" ~ "Juvenile xanthogranuloma",
...
ped_cancer_groups <- histology_cg %>%
  ...
  write_tsv("results/pediatric_cancer_groups.txt", col_names = F)
...
adult_cancer_groups <- histology_cg %>%
  ...
  tally() %>%
  arrange(cancer_group, tumor_descriptor) %>%
  write_tsv("results/tcga_cancer_groups.txt", col_names = F)
