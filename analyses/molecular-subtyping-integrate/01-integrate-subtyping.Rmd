...
# add broad_histology and short_histology for NBL-GMKF and TARGET samples
histology <- histology %>% 
  dplyr::mutate(
    short_histology = dplyr::case_when(
      pathology_diagnosis == "Neuroblastoma" ~ "Neuroblastoma",
      pathology_diagnosis == "Ganglioneuroblastoma, intermixed" ~ "Neuroblastoma",
      pathology_diagnosis == "Ganglioneuroblastoma, nodular" ~ "Neuroblastoma",
      pathology_diagnosis == "Ganglioneuroma, maturing subtype OR Ganglioneuroblastoma, well differentiated" ~ "Neuroblastoma",
      pathology_diagnosis == "Acute Lymphoblastic Leukemia" ~ "Acute Lymphoblastic Leukemia",
      pathology_diagnosis == "Acute Myeloid Leukemia" ~ "Acute Myeloid Leukemia",
      pathology_diagnosis == "Osteosarcoma" ~ "Osteosarcoma",
      pathology_diagnosis == "Kidney Rhabdoid Tumor" ~ "Rhabdoid tumor",
      pathology_diagnosis == "Kidney Wilms Tumor" ~ "Wilms tumor",
      pathology_diagnosis == "Clear Cell Sarcoma of the Kidney" ~ "Clear Cell Sarcoma of the Kidney",
      TRUE ~ as.character(short_histology)),
    broad_histology = dplyr::case_when(
      short_histology == "Neuroblastoma" ~ "Embryonal tumor",
      short_histology == "Acute Lymphoblastic Leukemia" ~ "Hematologic malignancy",
      short_histology == "Acute Myeloid Leukemia" ~ "Hematologic malignancy",
      short_histology == "Osteosarcoma" ~ "Osteosarcoma",
      short_histology == "Rhabdoid tumor" ~ "Renal tumor",
      short_histology == "Wilms tumor" ~ "Renal tumor",
      short_histology == "Clear Cell Sarcoma of the Kidney" ~ "Renal tumor",
      TRUE ~ as.character(broad_histology))
  )

# add broad_histology, short_histology, and cancer_group for TCGA samples
histology_other_cohorts <- histology %>% 
  dplyr::filter(cohort != "TCGA")
tcga_broad_histology <- 
  readr::read_tsv(file.path("input", "tcga_pathology_diagnosis_to_histology_mapping.tsv")) %>% 
  dplyr::rename(broad_histology_tcga = broad_histology,
                short_histology_tcga = short_histology,
                cancer_group_tcga = cancer_group)
histology_tcga_cohort <- histology %>% 
  dplyr::filter(cohort == "TCGA") %>% 
  dplyr::left_join(tcga_broad_histology, by = "pathology_diagnosis") %>% 
  dplyr::mutate(broad_histology = broad_histology_tcga,
                short_histology = short_histology_tcga,
                cancer_group = cancer_group_tcga) %>% 
  dplyr::select(-c("broad_histology_tcga", "short_histology_tcga", "cancer_group_tcga")) %>% 
  dplyr::bind_rows(histology_other_cohorts) %>% 
...
histology<- histology %>%
  ...
  dplyr::mutate(cancer_group = case_when(
    ...
    short_histology == "Neuroblastoma" ~ "Neuroblastoma",
    short_histology == "Acute Lymphoblastic Leukemia" ~ "Acute Lymphoblastic Leukemia",
    short_histology == "Acute Myeloid Leukemia" ~ "Acute Myeloid Leukemia",
    short_histology == "Osteosarcoma" ~ "Osteosarcoma",
    short_histology == "Rhabdoid tumor" ~ "Rhabdoid tumor",
    short_histology == "Wilms tumor" ~ "Wilms tumor",
    short_histology == "Clear Cell Sarcoma of the Kidney" ~ "Clear Cell Sarcoma of the Kidney",    
