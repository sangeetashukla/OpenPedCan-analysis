...
# Filter metadata to contain only samples that have TP53 scores
tp53_sample_id <- tp53_status %>% 
  pull(sample_id) %>% unique()

metadata <- metadata %>% 
  filter(sample_id %in% tp53_sample_id)
...
# Read in snv consensus and hotspots mutation data, filtering out LGAT
 - We will use the `snv-consensus-plus-hotspots.maf.tsv.gz` which contains consensus SNV calls that are present in all 3 callers (strelka2, mutect2, and lancet) and with hotspot rescued. 
