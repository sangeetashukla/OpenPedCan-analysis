...
> If there is no fusion found and both DNA and RNA are available for the samples and the cns_methylation_subclass_score >= 0.8, then classify as "IHG, To be classified." 
> If there is no fusion found and RNA is not available and the cns_methylation_subclass_score >= 0.8 or pathology_free_text_diagnosis says this is an IHG, then classify as "IHG, To be classified."
> If there is no fusion found and DNA and RNA are available and the cns_methylation_subclass_score < 0.8 and this sample was not previously subtyped as HGG, then remove from the HGG subtyping


```{r}

lgg_hgg_fusion <- read_tsv(file.path(root_dir, "analyses", "fusion-summary", "results", "fusion_summary_lgat_foi.tsv"))

NTRK_fusion <- colnames(lgg_hgg_fusion)[str_detect(colnames(lgg_hgg_fusion), "NTRK")]
NTRK <- lgg_hgg_fusion %>% 
  select(c("Kids_First_Biospecimen_ID", NTRK_fusion)) %>% 
  mutate(Sum = rowSums(lgg_hgg_fusion[, NTRK_fusion])) %>% 
  filter(Sum > 0) %>% 
  pull(Kids_First_Biospecimen_ID)


ALK_fusion <- colnames(lgg_hgg_fusion)[str_detect(colnames(lgg_hgg_fusion), "ALK")]
ALK <- lgg_hgg_fusion %>% 
  select(c("Kids_First_Biospecimen_ID", ALK_fusion)) %>% 
  mutate(Sum = rowSums(lgg_hgg_fusion[, ALK_fusion])) %>% 
  filter(Sum > 0) %>% 
  pull(Kids_First_Biospecimen_ID)

ROS1_fusion <- colnames(lgg_hgg_fusion)[str_detect(colnames(lgg_hgg_fusion), "ROS1")]
ROS1 <- lgg_hgg_fusion %>% 
  select(c("Kids_First_Biospecimen_ID", ROS1_fusion)) %>% 
  mutate(Sum = rowSums(lgg_hgg_fusion[, ROS1_fusion])) %>% 
  filter(Sum > 0) %>% 
  pull(Kids_First_Biospecimen_ID)

MET_fusion <- colnames(lgg_hgg_fusion)[str_detect(colnames(lgg_hgg_fusion), "MET")]
MET <- lgg_hgg_fusion %>% 
  select(c("Kids_First_Biospecimen_ID", MET_fusion)) %>% 
  mutate(Sum = rowSums(lgg_hgg_fusion[, MET_fusion])) %>% 
  filter(Sum > 0) %>% 
  pull(Kids_First_Biospecimen_ID)
...
IGH <- histologies_df %>% 
  ...
  select(Kids_First_Participant_ID, sample_id, pathology_diagnosis, pathology_free_text_diagnosis, cns_methylation_subclass, cns_methylation_subclass_score) %>%
  left_join(all_data_df, by = "sample_id")
...
IGH <- IGH %>% 
  mutate(molecular_subtype = case_when(
    ...
    !(Kids_First_Biospecimen_ID_DNA %in% RTK_list) & 
      ...
      !is.na(Kids_First_Biospecimen_ID_DNA) & 
      cns_methylation_subclass_score >= 0.8 ~ "IHG, To be classified", 
    ...
    !(Kids_First_Biospecimen_ID_DNA %in% RTK_list) & 
      ...
      !is.na(Kids_First_Biospecimen_ID_DNA) & 
      !is.na(Kids_First_Biospecimen_ID_DNA) & 
      cns_methylation_subclass_score < 0.8 | 
      sample_id %in% molecular_subtype_table[grep("HGG", molecular_subtype_table$molecular_subtype), "sample_id"] ~ "delete"
  )) %>% 
  select(c(Kids_First_Participant_ID.x, sample_id, Kids_First_Biospecimen_ID_DNA, Kids_First_Biospecimen_ID_RNA, molecular_subtype)) %>% 
  filter(molecular_subtype != "delete")

colnames(IGH) <- colnames(molecular_subtype_table)

molecular_subtype_table <- bind_rows(molecular_subtype_table, IGH)
