...
NTRK <- lgg_hgg_fusion %>% 
  select(c("Kids_First_Biospecimen_ID", colnames(lgg_hgg_fusion)[str_detect(colnames(lgg_hgg_fusion), "NTRK")])) 
NTRK <- NTRK %>% 
  mutate(Sum = rowSums(NTRK[, 2:ncol(NTRK)])) %>% 
  filter(Sum > 0) %>% 
  pull(Kids_First_Biospecimen_ID)

ROS1 <- lgg_hgg_fusion %>% 
  select(c("Kids_First_Biospecimen_ID", colnames(lgg_hgg_fusion)[str_detect(colnames(lgg_hgg_fusion), "ROS1")])) 
ROS1 <- ROS1 %>% 
  mutate(Sum = rowSums(ROS1[, 2:ncol(ROS1)])) %>% 
  filter(Sum > 0) %>% 
  pull(Kids_First_Biospecimen_ID)

ALK <- lgg_hgg_fusion %>% 
  select(c("Kids_First_Biospecimen_ID", colnames(lgg_hgg_fusion)[str_detect(colnames(lgg_hgg_fusion), "ALK")])) 
ALK <- ALK %>% 
  mutate(Sum = rowSums(ALK[, 2:ncol(ALK)])) %>% 
  filter(Sum > 0) %>% 
  pull(Kids_First_Biospecimen_ID)

MET <- lgg_hgg_fusion %>% 
  select(c("Kids_First_Biospecimen_ID", colnames(lgg_hgg_fusion)[str_detect(colnames(lgg_hgg_fusion), "MET")])) 
MET <- MET %>% 
  mutate(Sum = rowSums(MET[, 2:ncol(MET)])) %>% 
  filter(Sum > 0) %>% 
  pull(Kids_First_Biospecimen_ID)
