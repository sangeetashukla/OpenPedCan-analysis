...
```{r}
included_dna_specimens <- hgg_metadata_df %>%
...
```{r}
consensus_cnv_ids <- read_tsv(file.path(root_dir, 
                                        "data", 
                                        "consensus_wgs_plus_cnvkit_wxs.tsv.gz")) %>%
  pull(Kids_First_biospeicmen_ID) %>% 
  unique()
```

```{r}
all(included_dna_specimens %in% consensus_cnv_ids)
```
...
```{r}
sum(!(included_dna_specimens %in% consensus_cnv_ids))
```
