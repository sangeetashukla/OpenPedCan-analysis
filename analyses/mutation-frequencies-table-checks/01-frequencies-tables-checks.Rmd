...
changes_in_columns <- function(current_tables, previous_table, column) {
  # values specific to current table
  specific_to_current <- current_table %>% 
  dplyr::select(Dataset, column) %>% 
  dplyr::distinct() %>% 
    setdiff(previous_table %>% dplyr::select(Dataset, column) %>%
              dplyr::distinct()) %>% 
  ...
  specific_to_previous <- previous_table %>% 
    dplyr::select(Dataset, column) %>% 
    dplyr::distinct() %>% 
    setdiff(current_table %>% dplyr::select(Dataset, column) %>%
              dplyr::distinct()) %>% 
    dplyr::rename(previous_dataset = Dataset)
  # combine differences 
  changes_in_columns <- specific_to_current %>%
    dplyr::full_join(specific_to_previous, by = column) %>% 
    dplyr::select(column, current_dataset, previous_dataset)
