...
gtex_subgroups <-
  unlist(stringr::str_split(opt$gtex_subgroups, ','))
drop <- opt$drop
...
counts_object_filtered <-
  edgeR::filterByExpr(counts_object, group = DESeqGroup)
counts_object <-
...
seq_expr_set <-
  EDASeq::newSeqExpressionSet(
    ...
    phenoData = data.frame(bs_id, DESeqGroup, RNA_library, row.names = colnames(round(
...
dds_nobatch <-
  DESeq2::DESeqDataSetFromMatrix(
    ...
    colData =  data.frame(bs_id, DESeqGroup),
    design = design
  )
...
setDT(dge_output_nobatch.filt)
setkey(dge_output_nobatch.filt, cols = 'gene')

dfs.dge.filt <- lapply(dfs.dge.filt, function(x) {
  x <- x[, c('p_val_diff') := padj - dge_output_nobatch.filt$padj]
})
...
seq_expr_set_norm <-
  EDASeq::newSeqExpressionSet(
    ...
    phenoData = data.frame(bs_id, DESeqGroup, RNA_library, row.names = colnames(
