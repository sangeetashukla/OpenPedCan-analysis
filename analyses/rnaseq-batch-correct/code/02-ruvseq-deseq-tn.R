...
option_list <- list(
  ...
  make_option(opt_str = "--drop", type = "integer",
              help = "number of factors to drop for batch correction")
)
...
seq_expr_set <- EDASeq::newSeqExpressionSet(counts = round(counts_object$counts), phenoData = data.frame(bs_id, DESeqGroup, RNA_library, row.names = colnames(round(counts_object$counts))))
...
ruvg_test(seq_expr_set = seq_expr_set, k_val = c((drop+1):k_value), 
          emp_neg_ctrl_genes = emp_neg_ctrl_genes_normals, prefix = "hk_genes_normals", 
          diff_type = "deseq2", 
          output_dir = output_dir,
          plot_dir = plots_dir,
          design_variable = "DESeqGroup",
          color_var = "DESeqGroup", shape_var = "RNA_library", drop = drop)
...
dds <- DESeq2::DESeqDataSetFromMatrix(countData = round(counts_object$counts), colData =  data.frame(bs_id, DESeqGroup), design = design)
