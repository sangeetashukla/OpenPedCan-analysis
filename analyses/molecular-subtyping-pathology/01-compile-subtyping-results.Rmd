...
bind_exp_strategies <- function(results_df) {
  ...
  #               Kids_First_Biospecimen_ID_DNA columns, 
  #               and Kids_First_Biospecimen_ID_meth
  # Returns:
  #   A data.frame where each biospecimen is a row
  
  if("Kids_First_Biospecimen_ID_meth" %in% colnames(results_df)){
    
    rna_df <- results_df %>%
    dplyr::select(-Kids_First_Biospecimen_ID_DNA, -Kids_First_Biospecimen_ID_meth) %>%
    dplyr::filter(!is.na(Kids_First_Biospecimen_ID_RNA)) %>%
    dplyr::rename(Kids_First_Biospecimen_ID = Kids_First_Biospecimen_ID_RNA)
  
    dna_df <- results_df %>%
    dplyr::select(-Kids_First_Biospecimen_ID_RNA, -Kids_First_Biospecimen_ID_meth) %>%
    dplyr::filter(!is.na(Kids_First_Biospecimen_ID_DNA)) %>%
    dplyr::rename(Kids_First_Biospecimen_ID = Kids_First_Biospecimen_ID_DNA)
    
    meth_df <- results_df %>%
    dplyr::select(-Kids_First_Biospecimen_ID_RNA, -Kids_First_Biospecimen_ID_DNA) %>%
    dplyr::filter(!is.na(Kids_First_Biospecimen_ID_meth)) %>%
    dplyr::rename(Kids_First_Biospecimen_ID = Kids_First_Biospecimen_ID_meth)
    
    return(bind_rows(rna_df, dna_df, meth_df) %>% distinct())
...
# NBL does not merged into dev yet
#nbl_results_file <- file.path(nbl_dir, results_dir, "neuroblastoma-molecular-subtypes.tsv")
```
...
atrt_results_df <- bind_exp_strategies(atrt_results_df) %>%
  inner_join(select(histologies_df,join_cols),
             by = "Kids_First_Biospecimen_ID") %>%
  # will update subtype and integrated diagnosis in step 3
  mutate(integrated_diagnosis = NA_character_,
         broad_histology = "Atypical Teratoid Rhabdoid Tumor",
