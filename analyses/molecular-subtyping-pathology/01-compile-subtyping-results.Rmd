...
### Handling ATRT
```{r}
atrt_df <- data.frame(sample_id = NA, 
                      Kids_First_Participant_ID = NA, 
                      molecular_subtype = NA, 
                      Kids_First_Biospecimen_ID = NA)
for(x in colnames(atrt_results_df)[3:7]){
  temp <- atrt_results_df %>% 
    dplyr::select(sample_id, Kids_First_Participant_ID, molecular_subtype, x) %>% 
    dplyr::filter(!is.na(.[,x])) %>% 
    dplyr::rename(Kids_First_Biospecimen_ID = x)
  
  atrt_df <- bind_rows(atrt_df, temp)
}

atrt_results_df <-  atrt_df %>%
  inner_join(select(histologies_df,join_cols),
             by = "Kids_First_Biospecimen_ID") %>%
  # will update subtype and integrated diagnosis in step 3
